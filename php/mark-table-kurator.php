...
$where = "AND ";
if (!empty($_POST['filter-month-my-group'])) {
   $where = addWhere($where, "`mark`.`date` LIKE '".$_POST['filter-month-my-group']."%'", true);
}
if (!empty($_POST['filter-type-my-group'])) {
   $type = htmlspecialchars(implode(",", $_POST['filter-type-my-group']));
   $type = explode(',', $type);
   $x = 0;
   $where .= " AND (";
   while ($x <count($type)) {
