...
if (!empty($_POST['id'])  && !empty($_POST['dayStart']) && !empty($_POST['dayFinish'])) {
   ...
   try {
      ...
      WHERE `student`.`id` = :student_id AND ".$where."
      ORDER BY `date`",
         [
            ':student_id'     => $student_id,
            ':day_start'   => $day_start,
   ...
   while ($a < count($thead)) {
      ...
      try {
         ...
         FROM `visiting` INNER JOIN (`student` INNER JOIN `group` 
           ...
           ON `visiting`.`student`=`student`.`id`
         WHERE `student`.`id` = :student_id AND `date` = :date AND ".$where,
            [
               ':student_id'     => $student_id,
               ':date'        => $thead[$a]['date'],
               ':day_start'   => $day_start,
      ...
      try {
         $visit = $db->query("SELECT `student`.`surname`, `student`.`name`,
            ...
            FROM `visiting` INNER JOIN (`student` INNER JOIN `group` 
               ON `student`.`group`=`group`.`id`) 
            ON `visiting`.`student`=`student`.`id`
            WHERE `student`.`id` = :student_id AND `visiting`.`student` = :student AND ".$where."
